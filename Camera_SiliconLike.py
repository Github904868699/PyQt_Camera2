...
# ---------------------- SiliconUI-like Theme (QSS) ----------------------
# 说明：
# - 这是“参考 SiliconUI 视觉风格”的自定义 QSS，不依赖 siui 代码（避免强耦合/重构成本）。
# - 你只需要保持现有控件不变，就能得到接近 SiliconUI 的背景、按钮、卡片、下拉框、输入框等风格。

SILICON_LIKE_QSS = r"""
/* ===== Base ===== */
* { font-family: "Segoe UI", "Microsoft YaHei UI", "Microsoft YaHei", "PingFang SC"; font-size: 10pt; }
QWidget { color: #d0d7de; background: transparent; }
QWidget#CentralRoot { background-color: #0b1020; }

/* ===== Panels ===== */
QWidget#LeftPanel, QWidget#RightPanel { background: transparent; }
...
QLabel#StatusBar{
    ...
    border: 1px solid rgba(31, 42, 68, 180);
    border-radius: 12px;
    padding: 6px 10px;
    color: #e5e7eb;
}
...
QPushButton:hover{ background-color: #152243; border-color: #2c3f69; }
QPushButton:pressed{ background-color: #0c152c; border-color: #3b82f6; }
QPushButton:disabled{ background-color: rgba(17, 28, 54, 90); color: rgba(229, 231, 235, 90); border-color: rgba(37, 52, 86, 90); }

QPushButton[role="primary"]{
    background-color: #1d4ed8;
    border-color: #2563eb;
}
QPushButton[role="primary"]:hover{ background-color: #2563eb; border-color: #3b82f6; }
QPushButton[role="primary"]:pressed{ background-color: #1e40af; border-color: #60a5fa; }

QPushButton[role="danger"]{
    background-color: #b91c1c;
    border-color: #dc2626;
}
QPushButton[role="danger"]:hover{ background-color: #dc2626; border-color: #ef4444; }
QPushButton[role="danger"]:pressed{ background-color: #991b1b; border-color: #f87171; }

QPushButton[role="ghost"]{
    background-color: rgba(17, 28, 54, 60);
    border-color: rgba(37, 52, 86, 140);
}
QPushButton[role="ghost"]:hover{ background-color: rgba(21, 34, 67, 120); }
...
QComboBox:hover{ border-color: #2c3f69; }
QComboBox:focus{ border-color: #3b82f6; }
QComboBox::drop-down{
    subcontrol-origin: padding;
    subcontrol-position: top right;
    width: 28px;
    border-left: 1px solid rgba(34, 49, 84, 160);
...
QComboBox QAbstractItemView{
    ...
    outline: 0px;
    padding: 6px;
    selection-background-color: rgba(37, 99, 235, 140);
}
...
QDoubleSpinBox:focus, QSpinBox:focus{ border-color: #3b82f6; }
...
QCheckBox::indicator:hover{ border-color: #2c3f69; }
...
/* ===== Splitter ===== */
QSplitter::handle{
    background-color: rgba(31, 42, 68, 110);
}
QSplitter::handle:horizontal{ width: 6px; margin: 4px 2px; border-radius: 3px; }
...
def apply_silicon_like_theme(app: "QtWidgets.QApplication"):
    # 使用 Fusion 确保跨平台一致性，然后用 QSS 覆盖细节
    try:
        app.setStyle("Fusion")
    except Exception:
        pass
    try:
        ...
        pass
    app.setStyleSheet(SILICON_LIKE_QSS)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        for slot in (1, 2):
            ...
            gb_slot.setProperty(\"class\", \"card\")
            gb_slot.setAttribute(QtCore.Qt.WA_StyledBackground, True)
            ...
            btn_hik_refresh.setProperty("role", "ghost")
            ...
            btn_usb_refresh.setProperty("role", "ghost")
        ...
        gb_yolo.setProperty(\"class\", \"card\")
        gb_yolo.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        ...
        gb_modbus.setProperty(\"class\", \"card\")
        gb_modbus.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        ...
        self.video_lbl1.setProperty(\"class\", \"video\")
        ...
        self.video_lbl2.setProperty(\"class\", \"video\")
